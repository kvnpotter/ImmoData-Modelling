...
class DataPreprocessor:
    ...
    def view_correlations(self, data: None | str = None, columns: None | list = None) -> None:
        """
    ...
    def calc_vif(self, data: None | str = None, columns: None | list[str] = None) -> None:
        """
    ...
    def princomp(self, data: None | str = None, columns: None | list[str] = None, n_components: None | int = None) -> None:
        """
        ...
        explained_variance_df = pd.DataFrame({'Principal Component': [f'PC{i+1}' for i in range(len(explained_variance_ratio))],
                                              'Explained Variance Ratio': explained_variance_ratio,
                                              'Cumulative Explained Variance': np.cumsum(explained_variance_ratio)})

        explained_variance_df.to_excel(f'./Results-Graphs/PCA_{title_data}_{title_columns}_ExplainedVariance.xlsx', index= False)
        ...
        loadings.to_excel(f'./Results-Graphs/PCA_{title_data}_{title_columns}_Loadings.xlsx', index= True)
        print(loadings)
    ...
    def split_scale_train_test(self) -> None:
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.modelling_data_X, self.modelling_data_y, test_size= 0.2, random_state= 42)
    ...
    def label_encode_cat(self, category: str) -> None:
        ...
        self.data_income[new_col] = self.data_income[category].astype('category').cat.codes
