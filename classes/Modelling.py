...
class Modeller:
    ...
    def set_parameters(self,CV: bool = True, k : int = 5, distance: str = 'euclidian', weight: str = 'uniform') -> None:
        """
    ...
    def get_model(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        if type(self.model_params['n_neighbors']) == list:
            ...
            if self.gower == True:
                knn_gower(self.X_train, self.y_train)

            else:
                knn(self.X_train, self.y_train)
